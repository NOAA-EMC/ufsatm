module ufs_mpas_subdriver
  ...
  use module_mpas_config, only: pio_subsystems, fh_init, ic_file_path
...
contains
  ...
  !
  ! #########################################################################################
  subroutine ufs_mpas_init(Init, time_start, time_end, total_time, calendar)
    ...
    integer :: i, ndate1, ndate2, tod, ierr
    ...
    call mpas_framework_init_phase1(domain_ptr%dminfo, external_comm=Init%mpi_comm)
    ...
    if ( ierr /= 0 ) then
       call mpp_error(FATAL, "FATAL: Log setup failed for MPAS-A dycore")
    end if
    ...
    if (Init%me == Init%master) print*,'SWALES start_time ',ndate1,tod,date2yyyymmdd(ndate1)//'_'//sec2hms(tod)
    ...
    if (Init%me == Init%master) print*,'SWALES stop_time ',ndate2,tod,date2yyyymmdd(ndate2)//'_'//sec2hms(tod)
    ...
    if (Init%me == Init%master) print*,'SWALES run_time ',trim(int2str(1))//'_'//sec2hms(total_time)
...
!    call mpas_pool_add_config(domain_ptr%configs, 'config_pio_num_iotasks',        0)
!    call mpas_pool_add_config(domain_ptr%configs, 'config_pio_stride',             1)
    ...
    if (Init%me == Init%master) print*,'SWALES POST mpas_framework_init_phase2: ',pio_iotask_rank(pio_subsystem)
...
!       call mpp_error(FATAL, 'Streams file '//trim(domain_ptr % streams_filename)//' does not exist.')
!    end if
    
    ! Get the number of tracers.
    if (Init%me == Init%master) print*,'SWALES Init%nwat = ',Init%nwat

    ! Open MPAS Initial condition file.
    ic_file_path = "mpas.init.nc"
    allocate(fh_ini)
    fh_ini => fh_init(1)
!    ierr = pio_openfile(pio_subsystem, fh_ini, PIO_iotype_pnetcdf, ic_file_path, pio_nowrite)       
!    if (ierr /= pio_noerr) then
!       call mpp_error(FATAL, 'Opening MPAS initial condition file failed '//trim(ic_file_path))
!    end if
...
!    end if
    ...
    mesh_filename = "x1.40962.grid.nc"
